...
const PrimaryButton = ({ type = "button", children, className = "", loading = false, disabled = false, ...props }) => {
  const base =
  ...
  return (
    <button 
      type={type} 
      className={`${base} ${(loading || disabled) ? "opacity-60 cursor-not-allowed" : ""} ${className}`} 
