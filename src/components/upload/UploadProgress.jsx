    ...
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {fileSize && <p className="text-xs text-muted">{formatSize(fileSize)}</p>}
        </div>
      ...
      {showCancel && onCancel && (
        <button
          type="button"
          onClick={onCancel}
          className="mt-3 flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium text-text hover:bg-card"
        >
